...
| --- | --- | --- |
| Agent Cockpit | Implemented / Windows QA | Grid, Focus, and Timeline retain stable Terminal, Activity, Answer, and Evidence identity across concurrent sessions. Only captured observable output is shown. |
...
| Native Modern Workbench / Pixel compatibility | Supported | Native WebView2 Modern Workbench is the default Windows desktop and maps all twelve local Control Room pages. Pixel remains available through `--legacy-pixel`; neither surface downloads theme code or changes coordination authority. |
