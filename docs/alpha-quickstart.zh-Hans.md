...
PeerBridge 是本地优先、可审计的多 Agent 协作层。这个 Alpha 使用共享 SQLite
邮箱、任务租约、房间、互评、证据和 SHA 串联事件，让 Codex、Claude Code 或其他
已配置的 Agent 在不互相覆盖工作的情况下协作。
...
`PeerBridgeControlRoom-0.1.0a2-windows-x64-portable.zip`，完整解压到可写入的文件夹，
