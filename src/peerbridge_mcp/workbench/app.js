(() => {
  ...
  const translations = {
    "zh-Hant": {
      ...
      enqueue_workflow: "派發協作工作流程", workflow_hint: "選擇受治理的工作流程，PeerBridge 會建立可追蹤的執行記錄。", workflow: "工作流程", max_attempts: "最多嘗試", timeout_seconds: "逾時秒數", task_description: "工作內容", task_description_placeholder: "說明目標、限制及完成條件", enqueue: "加入佇列", workflow_enqueued: "工作流程已加入佇列", cancel_operation: "取消執行", operation_cancelled: "已要求取消執行",
    ...
    "zh-Hans": {
      ...
      enqueue_workflow: "派发协作工作流", workflow_hint: "选择受治理的工作流，PeerBridge 会建立可追踪的执行记录。", workflow: "工作流", max_attempts: "最多尝试", timeout_seconds: "超时秒数", task_description: "工作内容", task_description_placeholder: "说明目标、限制和完成条件", enqueue: "加入队列", workflow_enqueued: "工作流已加入队列", cancel_operation: "取消执行", operation_cancelled: "已请求取消执行",
  ...
  function renderOperations() {
    ...
    rows.forEach((entry) => {
      ...
      const meta = node("div", "record-meta"); [entry.workflow_id, entry.requested_by, `${t("attempt")} ${entry.attempt_count || 0}/${entry.max_attempts || 0}`, timeLabel(entry.updated_utc)].filter(Boolean).forEach((value) => meta.append(node("span", "", value))); row.append(meta);
  ...
  function renderWorkflowControls() {
    ...
    replaceSelectOptions(select, templates.map((entry) => [entry.workflow_id, entry.label || entry.workflow_id]), prior);
    ...
    replaceSelectOptions(byId("schedule-workflow"), templates.map((entry) => [entry.workflow_id, entry.label || entry.workflow_id]), byId("schedule-workflow")?.value);
  }
